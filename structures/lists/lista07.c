...
void copia(No ** lista, No ** destino) {
  No * tmp = *lista;

  while (tmp) {
    No * elem = buscar(destino, tmp->valor);

    if (!elem) {
      inserir_no_fim(destino, tmp->valor);
    }
...
int main() {
  ...
  inserir_no_inicio(&listaB, 4);
  inserir_no_inicio(&listaB, 5);
  inserir_no_inicio(&listaB, 6);
  inserir_no_inicio(&listaB, 1);
